	...
	public static class Main_Update_CreateWandHotkeysPatch
	{
		...
		public static void Postfix(GameTime gameTime)
		{
			...
			try
			{
				...
				if (_giveCooldown == 0 && KeyPressed(kb, prevKb, Keys.J))
					GiveCreateWand(p);
					...
					else if (KeyPressed(kb, prevKb, Keys.F9))
					{
						...
						if (shift)
						{
							CreateWandSelectionState.OutgoingNetTraceTileRelatedOnly =
								!CreateWandSelectionState.OutgoingNetTraceTileRelatedOnly;
							Terraria.CombatText.NewText(p.getRect(), new Color(200, 230, 255),
								CreateWandSelectionState.OutgoingNetTraceTileRelatedOnly
				...
				// Android visible touch zone UI
				CreateWandTouchUI.ProcessTouchInput();

				// Handle speed zone tap (split left/right)
				if (Main.mouseLeft && !_lastKb.IsKeyDown(Keys.OemComma) && !_lastKb.IsKeyDown(Keys.OemPeriod))
				{
					// Speed zone handled inside ProcessTouchInput via coordinate check
				}
				...
				if (KeyPressed(kb, prevKb, Keys.OemComma))
				{
					...
					Terraria.CombatText.NewText(p.getRect(), new Color(200, 220, 255),
				...
				{
					int newSpeed = CreateWandStaggeredPlacementQueue.AdjustHandheldSpeed(-1);
					Terraria.CombatText.NewText(p.getRect(), new Color(200, 255, 200),
				...
				else if (KeyPressed(kb, prevKb, Keys.K))
				{
					...
					{
						Terraria.CombatText.NewText(p.getRect(), Color.OrangeRed,
					...
					else if (TryPackFastClear(p, out BuildingData data, out int ox, out int oy))
					{
						...
						Terraria.CombatText.NewText(p.getRect(), Color.Orange,
				...
				if (KeyPressed(kb, prevKb, Keys.B))
				{
					if (CreateWandBoxExportState.Phase != CreateWandBoxExportPhase.Inactive)
					{
						...
						Terraria.CombatText.NewText(p.getRect(), Color.Gray,
							"[魔杖] 已取消框选导出", false, false);
					}
					else
					{
						CreateWandBoxExportState.Begin();
						Terraria.CombatText.NewText(p.getRect(), Color.LightYellow,
				...
				else if (KeyPressed(kb, prevKb, Keys.O))
				{
					...
					Terraria.CombatText.NewText(p.getRect(), new Color(255, 200, 100),
				...
				else if (KeyPressed(kb, prevKb, Keys.OemOpenBrackets))
				{
					...
					Terraria.CombatText.NewText(p.getRect(), new Color(176, 196, 222),
				...
				else if (KeyPressed(kb, prevKb, Keys.OemSemicolon))
				{
					...
					Terraria.CombatText.NewText(p.getRect(), new Color(200, 230, 180),
				...
				else if (KeyPressed(kb, prevKb, Keys.OemBackslash) || KeyPressed(kb, prevKb, Keys.OemPipe))
				{
					...
					Terraria.CombatText.NewText(p.getRect(),
						CreateWandSelectionState.PlacementEnabled ? Color.LightGreen : Color.Gray,
				...
				if (KeyPressed(kb, prevKb, Keys.D1))
				{
					...
					Terraria.CombatText.NewText(p.getRect(), Color.LightBlue, "[魔杖] 预设：石块 3×3", false, false);
					CreateWandWorldPreview.InvalidateCache();
				}
				else if (KeyPressed(kb, prevKb, Keys.D2))
				{
					...
					Terraria.CombatText.NewText(p.getRect(), Color.LightBlue, "[魔杖] 预设：木平台×5", false, false);
					CreateWandWorldPreview.InvalidateCache();
				}
				else if (KeyPressed(kb, prevKb, Keys.D3))
				{
					...
					Terraria.CombatText.NewText(p.getRect(), Color.LightBlue, "[魔杖] 预设：泥土 2×2", false, false);
					CreateWandWorldPreview.InvalidateCache();
				}
				else if (KeyPressed(kb, prevKb, Keys.OemMinus) || KeyPressed(kb, prevKb, Keys.Subtract))
				{
					...
					if (CreateWandPngLibrary.Entries.Count > 0)
					{
						...
						Terraria.CombatText.NewText(p.getRect(), Color.LightGreen, "[魔杖] 蓝图：" + e.Name, false, false);
					}

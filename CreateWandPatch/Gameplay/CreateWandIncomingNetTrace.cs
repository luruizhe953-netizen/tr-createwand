...
{
	/// <summary>
	/// 联机客户端钩住 <see cref="Terraria.MessageBuffer.GetData"/> 时的文本日志（服务器→本机，与魔杖无关）。
	/// 开关与路径逻辑与 <see cref="CreateWandOutgoingNetTrace"/> 对齐：同一 <see cref="CreateWandSelectionState.EnableClientOutgoingNetTrace"/>（F9）。
	/// </summary>
	internal static class CreateWandIncomingNetTrace
	{
		...
		private static string ResolveLogPath()
		{
			...
			try
			{
				string envFull = Environment.GetEnvironmentVariable("CREATEWAND_INCOMING_LOG_PATH");
				...
				string dirVar = Environment.GetEnvironmentVariable("CREATEWAND_INCOMING_LOG_DIR");
				if (!string.IsNullOrWhiteSpace(dirVar))
				{
					dirVar = dirVar.Trim().Trim('"');
					Directory.CreateDirectory(dirVar);
					_cachedPath = Path.Combine(dirVar, "CreateWandPatch-incoming-net.log");
					return _cachedPath;
				}
				...
				_cachedPath = Path.Combine(baseDir, "CreateWandPatch-incoming-net.log");
				return _cachedPath;
			}
			catch
			{
